...
import { json, error, preflight, ownerHash, readJson, ratingOf, nowMs } from "../../_shared.js";
...
export async function onRequestPost({ request, params, env }) {
  ...
  if (!token) return error("Missing X-GG-Token — a client token is required to rate", 401);
  ...
  if (prev) {
    ...
    await env.DB.batch([
      env.DB.prepare(`UPDATE ratings SET stars = ?, created_at = ? WHERE preset_id = ? AND token = ?`).bind(stars, ts, params.id, token),
      env.DB.prepare(`UPDATE presets SET rating_sum = rating_sum + ? WHERE id = ?`).bind(delta, params.id),
    ]);
  } else {
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO ratings (preset_id, token, stars, created_at) VALUES (?, ?, ?, ?)`).bind(params.id, token, stars, ts),
